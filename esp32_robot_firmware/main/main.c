...
#define D_WHEEL 0.065f
#define L_TRACK 0.135f
#define TICKS_PER_REV 300.0f
...
const float FILTER_ALPHA = 0.90f;

const float FF_ESQ = 0.085f;
const float FF_DIR = 0.075f;

const float SYNC_GAIN = 0.05f;
const float OFFSET_BASE = 7.0f;
const float MAX_I_TERM = 8.0f;
...
void control_task(void *pvParameters)
{
    ...
    while (1)
    {
        ...
        float dist_centro = (de_m + dd_m) / 2.0f;
        float delta_theta = (dd_m - de_m) / L_TRACK;
...
void app_main(void)
{
    ...
    xTaskCreatePinnedToCore(control_task, "PID_CTRL", 4096, NULL, 10, NULL, 1);
    xTaskCreatePinnedToCore(micro_ros_task, "ROS_TASK", 10240, NULL, 5, NULL, 0);
}